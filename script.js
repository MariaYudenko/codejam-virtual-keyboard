...
const Keyboard = {
  row1: [{keyCode: '192', name: 'Backquote', simbols: ['ё','Ё','`','~'], class: 'key'},
         ...
         {keyCode: '189', name: 'Minus', simbols: ['-','_','-','_'], class: 'key'},
         {keyCode: '187', name: 'Equal', simbols: ['=','+','=','+'], class: 'key'},
         {keyCode: '8', name: 'Backspace', simbols: ['Backspace','Backspace','Backspace','Backspace'], class: 'key backspace'},
        ],
  row2: [{keyCode: '9', name: 'Tab', simbols: ['Tab','Tab','Tab','Tab'], class: 'key tab'},
         ...
         {keyCode: '219', name: 'BracketLeft', simbols: ['х','Х','[','{'], class: 'key'},
         {keyCode: '221', name: 'BracketRight', simbols: ['ъ','Ъ',']','}'], class: 'key'},
         {keyCode: '220', name: 'Backslash', simbols: ['\\','/','\\','|'], class: 'key'},
         {keyCode: '46', name: 'Delete', simbols: ['DEL','DEL','DEL','DEL'], class: 'key del'}
        ],
  row3: [{keyCode: '20', name: 'CapsLock', simbols: ['CapsLock','CapsLock','CapsLock','CapsLock'], class: 'key capslock'},
         ...
         {keyCode: '186', name: 'Semicolon', simbols: ['ж','Ж',';',':'], class: 'key'},
         {keyCode: '222', name: 'Quote', simbols: ['э','Э','\'','\"'], class: 'key'},
         {keyCode: '13', name: 'Enter', simbols: ['ENTER','ENTER','ENTER','ENTER'], class: 'key enter'}
        ],
  row4: [{keyCode: '16', name: 'ShiftLeft', simbols: ['Shift','Shift','Shift','Shift'], class: 'key shift'},
         ...
         {keyCode: '188', name: 'Comma', simbols: ['б','Б',',','<'], class: 'key'},
         {keyCode: '190', name: 'Period', simbols: ['ю','Ю','.','>'], class: 'key'},
         {keyCode: '191', name: 'Slash', simbols: ['.',',','/','?'], class: 'key'},
  ...
  row5: [{keyCode: '17', name: 'ControlLeft', simbols: ['Ctrl','Ctrl','Ctrl','Ctrl'], class: 'key ctrl'},
         {keyCode: '91', name: 'MetaLeft', simbols: ['Win','Win','Win','Win'], class: 'key win'},
...
function init() {
  ...
  for ( line in Keyboard) {
    ...
    for (let i = 0; i < Keyboard[line].length; i++) {
      ...
      container.innerHTML = '<span class=\"' + Keyboard[line][i]['name'] + 
        off + '\"><span class=\"case down\">' + Keyboard[line][i]['simbols'][0] + '</span>' +
        '<span class=\"case up\">' + Keyboard[line][i]['simbols'][1] + '</span>';
      container.innerHTML += '<span class=\"' + Keyboard[line][i]['name'] + 
        on + '\"><span class=\"case down\">' + Keyboard[line][i]['simbols'][2] + '</span>' +
        '<span class=\"case up\">' + Keyboard[line][i]['simbols'][3] + '</span>';
          ...
          for (let i = 0; i < Keyboard[el].length; i++) {
            if (EO.keyCode == Keyboard[el][i]['keyCode']) {
              if (( !pressed.has("Shift") && capslock ) || ( pressed.has("Shift") && !capslock )) {
                strTextArea = (lang == 'en') ? Keyboard[el][i]['simbols'][3] : Keyboard[el][i]['simbols'][1];
              } else if ((!pressed.has("Shift") && !capslock) || (pressed.has("Shift") && capslock)) {
                strTextArea = (lang == 'en') ? Keyboard[el][i]['simbols'][2] : Keyboard[el][i]['simbols'][0];
              }
...
window.addEventListener('click', function(EO) {
  ...
  if((EO.target.className == 'case down') || (EO.target.className.indexOf('key') == 0  && EO.target.className.indexOf('keyboard') == -1)) {
    console.log('__'+EO.target.innerText+'__');
